    ...
    module.registerController("galleryCtrl", function ($scope, $compile, $window, $q, $http, $stateParams, $localStorage, Mediator, coreService) {
		...
		var getItems =  function(){
			...
			if($scope.context.action === "search_items") {
				switch($scope.context.filter_by.entity) {
					case "people":
						service = coreService.TaggedItem; 
						params["id"] = $scope.context.filter_by.params.selected[0].id;
						break;
					default: 
						service = coreService.FilteredItem; 
						params["keywords"] = $scope.context.filter_by.params.keywords;
						break;
				}
			}
		...
		var infobox = '<div id="imgInfoBox" class="superbox-imageinfo inline-block col-xs-12 col-sm-5 col-md-5 col-lg-5">' + 
						...
						'<span>' + 
							...
							'<p>' + 
								...
								' <a ng-href="' + appConfig.coreApiUrl + 'items/file/{{currentItem.id}}/?type=original&token=123456" class="btn btn-sm btn-success">Download</a>' +
							'</p>' + 
						'</span>' + 
					'</div>';
