...
def createTagKeyword(item_id, first_name, last_name):
    """
    ...
    """
    k1 = create_keyword(first_name)
    k2 = create_keyword(last_name)
    
    t1 = create_tag(item_id, k1['id'], 'keyword')
    t2 = create_tag(item_id, k2['id'], 'keyword')


def speaker_diarization_video(func_in, func_out):
    try:
        cmd = '/usr/bin/ffmpeg -y -i "' + os.path.join(settings.MEDIA_ROOT, func_out["file"]) + '"'
        cmd += ' -strict -2 -acodec pcm_s16le -ac 1 -ar 16000 '
        cmd += os.path.join(settings.MEDIA_ROOT, 'items', str(func_out['id']), 'audio.wav')
        print cmd
        subprocess.call(cmd, shell=True)
        func_out['file'] = os.path.join('items', str(func_out['id']), 'audio.wav')
        print func_out
        speaker_diarization(func_in, func_out)
    except:
	...
	try:
		...
		if name.find("GiacomoMameli")>-1:
		    ...
		    id_persona=create_person("Giacomo","Mameli")["id"]
                    createTagKeyword(id_item, 'Giacomo', 'Mameli')
			...
			s_name=mai[1]+name.split(mai[1])[1]
                        ...
                        persona = None
                        if f_name == "Unknow":
			    persona=create_person(f_name,s_name+'_'+str(id_item))
                        else:
                            persona=create_person(f_name,s_name)
                        
                        # create a tag for person name
                        createTagKeyword(id_item, persona['first_name'], persona['last_name'])
