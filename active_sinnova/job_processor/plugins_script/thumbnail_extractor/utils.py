...
def extract_video_thumbnail(func_in, func_out):
    ...
    res = set_thumbnail(func_out['id'], thumb_path, 'image/jpeg')

    # delete the temporary thumbnail from local filesystem
    os.remove(thumb_path)

    if not res:
        raise Exception("Error on video thumbnail generation " + str(func_out['id']))
...
def extract_audio_thumbnail(func_in, func_out):
    ...
    if not res:
        raise Exception("Error on thumbnail generation for audio item " + str(func_out['id']))
