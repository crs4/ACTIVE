...
class FaceModels:
                ...
                if det_results and (c.FACES_KEY in det_results):
                    ...
                    if ((len(faces) == 1) and (c.FACE_KEY in faces[0]) and
                        ...
                        if c.USE_NORM_IN_CROPPED_FACES:
                            ...
                            aligned_face = cv2.normalize(aligned_face,
                        ...
                        if c.USE_TAN_AND_TRIGG_NORM:
                            aligned_face = utils.normalize_illumination(
                                aligned_face)

                        # Insert oval mask in image
                        if c.USE_OVAL_MASK:
                            aligned_face = utils.add_oval_mask(aligned_face)
    ...
    def create_models(self):
        ...
        Folder with aligned faces contains one sub folder for each person
        """
