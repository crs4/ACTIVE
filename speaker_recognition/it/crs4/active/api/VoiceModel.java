...
/**
 * The package it.crs4.active.api contains all of the classes for creating a user model and for identify a person from an audio file.<br>
 * Using this class is possible:<br> 
 * <br> <li>add an audio segment spoken by a single speaker (and a tag identifying the speaker) to the training 
 * set of the speaker recognition tool
 ...
 *  the class VoiceModel is used for build the voice models used by the 
 *  voice recognition algorithm <br>

   
   <br> <strong>An Example: build a model</strong>
   <code>
<br>String configuration="example.properties";
<br>String filename="example.wav";
<br>String tag="model_name";
<br>VoiceModel voicemodel=new VoiceModel(configuration);
<br>voicemodel.addVoice(filename, tag);
</code>
