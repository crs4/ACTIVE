...
class FaceModels():
    ...
    def add_face(self, tag, im_path, aligned_face = None, eye_pos = None, bbox = None):
        ...
        try:
            ...
            if(aligned_face and eye_pos and bbox):
            ...
            if(good_image): 
                ...
                # Load file with eye positions
                faces_file = os.path.join(
                whole_images_path, c.FACES_FILE)
                ...
                print('whole_im_path', whole_im_path)
    ...
    def get_tags(self):
        '''
        Get all tags as a list of strings
        '''
    ...
    def recognize_face(self, face):
        ...
        if(self._models):
            ...
            #query_model = cv2.createLBPHFaceRecognizer(1, 8, 4, 8)
            #query_model.train(np.asarray([np.asarray(face, dtype=np.uint8)]), 
            #np.asarray(0))
            #query_hists = query_model.getMatVector("histograms")      
            #query_hist = query_hists[0][0]
            #train_hists = self._models.getMatVector("histograms")
            #train_labels = self._models.getMat("labels") 
            #min_distance = sys.maxint
            #min_label = -1
            #for i in range(0, len(train_hists)):    
                #train_hist = train_hists[i][0]
                #diff = cv2.compareHist(
                #query_hist, train_hist, cv2.cv.CV_COMP_CHISQR)

                #if(diff < min_distance):
                    #min_distance = diff
                    #min_label = train_labels[i][0]
            #print('min label', min_label)
            #print('conf', min_distance)             
            #print('\nWith normalization')
            #query_model = cv2.createLBPHFaceRecognizer(1, 8, 4, 8)
            #query_model.train(np.asarray([np.asarray(face, dtype=np.uint8)]), 
            #np.asarray(0))
            ...
            #query_hist = query_hists[0][0]
            ## Normalization
            #for j in range(0, len(query_hist)):
                #query_hist[j] = query_hist[j]/32.0
            #raw_input('...')
            #train_hists = self._models.getMatVector("histograms")
            #train_labels = self._models.getMat("labels") 
            #min_distance = sys.maxint
            ...
            #for i in range(0, len(train_hists)):    
                #train_hist = train_hists[i][0]
                ## Normalization
                #for j in range(0, len(train_hist)):
                    #train_hist[j] = train_hist[j]/32.0
                #diff = cv2.compareHist(
                #query_hist, train_hist, cv2.cv.CV_COMP_CHISQR)
                #if(diff < min_distance):
                    #min_distance = diff
