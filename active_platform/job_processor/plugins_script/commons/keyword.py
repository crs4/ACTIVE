...
def get_keyword_by_item(item_id, token=None):
    """
    Method used to retrieve all keywords associated with a specific item.
    A object JSON serialized is returned, if any.

    @param item_id: Id of the item.
    @param token: Authentication token necessary to invoke the REST API.
    @return: The keyword list, None in case of error.
    """
    url = settings.ACTIVE_CORE_ENDPOINT + 'api/keywords/item/' + str(item_id)
    header = {'Authorization': token}
    r = requests.get(url, headers=header)
...
def search_keyword(keyword_value, token=None):
    ...
    @return: The keyword object, None in case of error.
    """
    url = settings.ACTIVE_CORE_ENDPOINT + 'api/search/keywords/' + keyword_value
    header = {'Authorization': token}
    r = requests.get(url, headers=header)
