...
def create_model(entity_id, type, name=None, model_file_path=None, last_update=None, token=None):
    """
    ...
    @type type: String
    @param model_file_path: The path of the file representing the model
    ...
    url = settings.ACTIVE_CORE_ENDPOINT + 'training_set_manager/models/'
    header = {'Authorization': token}
    model = {'name': name, 'type': type, 'entity': entity_id, 'model_file': model_file_path, 'last_update': last_update}
...
def edit_instance(instance_id, type=None, trusted=None, token=None):
    """
