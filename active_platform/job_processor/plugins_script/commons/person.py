...
def get_person_by_item(item_id, token=None):
    """
    Method used to retrieve all data of a person associated with a specific Item.
    
    @param item_id: Id of the item.
    @param token: Authentication token necessary to invoke the REST API.
    @return: Object containing all person data.
    """
    people = set()
    ...
    for tag in tags:    
        url = settings.ACTIVE_CORE_ENDPOINT + 'api/people/' + str(tag['entity']) + '/'        
        r = requests.get(url, headers=header)
        # check if the person has been retrieved correctly
        if r.status_code == requests.codes.ok:
            if "Unknown" not in r.json()['first_name']:
                people.add(r.json()['first_name'] + " " + r.json()['last_name'])          
    return list(people)
...
def get_person_by_name(name, surname, token=None):
    ...
    :return: Object containing all person data, None in case of error.
    """
    url = settings.ACTIVE_CORE_ENDPOINT + 'api/search/person/' + name + '/' + surname + '/'
    header = {'Authorization': token}
    r = requests.get(url, headers=header)
