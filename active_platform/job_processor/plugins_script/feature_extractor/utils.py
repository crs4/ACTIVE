...
def extract_video_data(auth_params, func_params):
    """
    ...
    if not set_video_metadata(func_params['id'], item_info, token):
    ...
    print "Extracted and saved metadata for video item", func_params['id']


def extract_image_data(auth_params, func_params):
    """
    ...
    if not set_image_metadata(func_params['id'], item_info, token):
        raise Exception('Error on metadata update')
...
def extract_audio_data(auth_params, func_params):
    """
    This function is used to extract relevant metadata from a video item
    and then save this data on the active core.
    ...
    if not set_audio_metadata(func_params['id'], item_info, token):
        raise Exception('Error on metadata update')
