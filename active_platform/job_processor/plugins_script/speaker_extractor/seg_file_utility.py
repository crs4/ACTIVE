...
def segfile_compact_name2(file_path):
    result=[]
    res_file=open(file_path+".tag2.res.txt", "w")
    
    with open(file_path+".h.3.seg", "r") as f:
         name=None
         start=None
         dur=None

         lines         = f.readlines()
         cluster_lines = [ l for l in lines if not l.startswith(';;')]
         occurrences   = [ (l.split(' ')[2], l.split(' ')[3], l.split(' ')[7]) for l in cluster_lines]
         for o in occurrences:
		     ...
		     print "durata ",float(start)+float(dur), " vs ", float(o[0])
                     if float(start)+float(dur)==float(o[0]) and name==o[2]:
                         dur=float(dur)+float(o[1])
                         print "**dur ", dur
                     else:
    ...
    result.append([name, start, dur])
    res_file.close()
    print "result segfile_compact_name2 ", result
    return result
