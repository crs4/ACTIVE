...
#            print str(self._diar_conf[1])
    ...
    def extract_cluster(self, interactive=False, quiet=False, thrd_n=16):
    ...
    def extract_speakers_without_diarization(self, interactive=False, quiet=False, thrd_n=16):
        ...
        # search for every identified cluster if there is 
        # a relative model voice in the db
        self._cluster_matching_no_sox(0, interactive, quiet, thrd_n,
                               start_time)
            ...
            if not quiet:
                ...
                if diarization_time != None:
                    ...
                    print """\nwav duration: %s\nall done in %dsec (%s) (diarization %dsec time:%s )  with %s threads and %d voices in db (%f) """ % (utils.humanize_time(sec),
                                                                                                                                                  ...
                                                                                                                                                  tot_voices,
                                                                                                                                                   t_f_s)
    def _cluster_matching_no_sox(self, diarization_time=None, interactive=False, quiet=False, thrd_n=1, start_t=0):
        """Match for voices in the db"""
        basename = self.get_file_basename()
        print "SR.PY 1009--- basename ", basename
        self._status = 4
        ...
        if not interactive:
            if not quiet:
                ...
                if diarization_time != None:
                    ...
                    print "\nwav duration: %s\nmatch clusters done in %dsec (%s)  with %s threads and %d voices in db iii" % (utils.humanize_time(sec),total_time,tot_voices)
